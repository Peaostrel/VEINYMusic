...
- **Discord Rich Presence (Игровой статус)**:
  ...
  - **Высококачественные обложки**: Загрузка оригинальных обложек альбомов в высоком разрешении напрямую с серверов Яндекса.

- **Smart Debouncing (Сглаживание при переключении)**:
  - При быстром пролистывании треков скрипт не спамит запросами к Discord RPC и к серверам Яндекса. Обновление статуса в Discord и поиск обложек происходят только после того, как трек проиграется без переключений более **1.5 секунд**.
  - При этом терминальный интерфейс (TUI) реагирует на переключения мгновенно.
...
- **Smart Auto-Update (Автообновление)**: 
  - Скрипт автоматически сверяет версию с репозиторием GitHub при старте (только если запущен как одиночный файл без папки `.git`). Если доступна новая версия, он предложит обновиться в один клик.
...
Для получения 100% точных обложек альбомов и названий треков напрямую через официальный API Яндекса (без использования сторонних баз Deezer/iTunes):
